...
static void *limo_threading_entry_function(void *thread_thunk)
{
  ...
  flmalloc_lists = GC_malloc(MAX_FLMALLOC_LISTS * sizeof (void *));
  ...
  GC_reachable_here(flmalloc_lists);
  GC_reachable_here(make_limo_data_next);
  GC_reachable_here(stacktrace);
  GC_reachable_here(make_stacktrace_free);
  return NULL;
}
